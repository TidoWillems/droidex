    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <span className="glow-orange text-orange-400 font-bold text-sm tracking-widest uppercase">REBIRTH</span>
          ...
          {nextRebirth ? (
            <span className="text-zinc-500 text-xs flex items-center gap-0.5">
              ...
              <span>Rebirth <span className="text-zinc-300 font-bold">{nextRebirth.to}</span></span>
            </span>
          ) : (
            <span className="text-yellow-400 text-xs font-bold tracking-wide">MAX REBIRTH</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {nextRebirth && (
            allMet ? (
              <span className="glow-green text-xs font-bold text-green-400">✓ READY</span>
            ) : (
              <div className="flex flex-col items-end gap-0.5">
                <span className="text-xs font-bold text-red-400">{ownedCount}/{nextRebirth.droids.length} droids</span>
                <div className="flex gap-0.5">
                  {Array.from({ length: nextRebirth.droids.length }, (_, i) => (
                    <div key={i} className={`w-1.5 h-1 rounded-full ${i < ownedCount ? 'bg-orange-500' : 'bg-zinc-700'}`} />
                  ))}
                </div>
              </div>
            )
      ...
      {open && nextRebirth && (
        <div className="px-4 pb-4 pt-1">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="need-label text-[10px] font-black tracking-[0.3em] uppercase">NEED</span>
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <div className="flex flex-col items-center gap-1">
                ...
                <div className="credits-card relative w-[88px] h-[88px] rounded-xl border-2 border-amber-500/70 flex flex-col items-center justify-center gap-0.5 overflow-hidden">
                  ...
                  <span className="text-amber-400 text-base leading-none relative z-10">◎</span>
                  <span className="text-amber-400 font-black text-base leading-tight text-center px-1 relative z-10">{nextRebirth.credits}</span>
                  <span className="text-amber-600 text-[8px] uppercase tracking-widest relative z-10">credits</span>
                </div>
                <span className="text-zinc-400 text-[10px] font-bold w-[88px] text-center truncate">Credits</span>
              </div>
              ...
              {nextRebirth.droids.map((d) => {
                ...
                return (
                  <div key={d.cardId} className="flex flex-col items-center gap-1">
                    ...
                    {/* Card */}
                    <div className={`relative w-[88px] h-[88px] rounded-xl border-2 overflow-hidden bg-zinc-900 ${have ? 'droid-card-owned' : 'droid-card-missing'}`}>
                      {/* Droid image */}
                      <img
                      ...
                      <div className="absolute bottom-0 left-0 right-0 text-center py-0.5 bg-black/60">
                        <span className={`text-[8px] font-black uppercase tracking-wide ${TIER_CLASS[d.tier] ?? 'text-gray-400'}`}>
                          {d.tier}
                        </span>
                      </div>

                      {/* Check / X circle — top left */}
                      <div className={`absolute top-1 left-1 w-5 h-5 rounded-full flex items-center justify-center ${have ? 'bg-green-500' : 'bg-red-500'}`}>
                        {have ? (
                    ...
                    {/* Name below */}
                    <span className="text-white text-[10px] font-bold w-[88px] text-center truncate">{d.name}</span>
                  </div>
                )
              })}
            </div>
