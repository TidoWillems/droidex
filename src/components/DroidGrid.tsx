...
export function DroidGrid({ tier, rarity, droidClass, collectionStatus, search, collected, onToggle, highlightedIds }: Props) {
  ...
  const cards = useMemo(() => {
    const filtered = ALL_CARDS.filter((c) => {
      ...
      if (search.trim() && !c.droid.name.toLowerCase().includes(search.trim().toLowerCase())) return false
      return true
    })
    if (tier === 'ALL') {
    ...
    return filtered
  }, [tier, rarity, droidClass, collectionStatus, search, collected, tierIndex])
